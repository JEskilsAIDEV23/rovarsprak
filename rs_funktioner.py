...
def text_imp(fil_r, frc = 0.5):
 ...
 #           if (check_char(text_r, frc)) == True: #kontrollerar textinnehållet m.a.p. tecken innehåll
            status, n_c, n_sum = check_char(text_r, frc)
            if status == True:
                return text_r, fil_r, n_c, n_sum
 ...
 #               exit()
    ...
    if check_path(fil_r) == False:  
        ...
        text_r, fil_r, n_c, n_sum = text_imp(fil_n, frc) #rekursivt anrop av text_imp() fil_r motsvaras nu av fil_n
